...
def should_try(date_slash):
    ...
    註: 補班制度已於 2025 下半年取消(人事行政總處修正補假要點, 改為只補假不補班),
        2026 起不會再有新的補行交易日。這裡的週六邏輯是為了「重跑歷史回補時
        不會又把那 23 天弄丟」而保留, 不是為了未來。
        因此 scrape.is_trading_day 與 daily.yml 的 cron(週一~五)維持原樣即可。
    """
    global _HOLIDAYS
